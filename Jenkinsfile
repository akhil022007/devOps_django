     ...
     stages {
         ...
         stage('Deploy Application') {
             steps {
                 ...
                 withCredentials([string(credentialsId: 'DJANGO_SECRET_KEY_CREDENTIAL', variable: 'DJANGO_SECRET_KEY_VAR')]) {
                     script {
                        // For docker-compose exec, variables are typically available from the container's environment
                        // which was set during 'docker-compose up'. However, to be absolutely safe,
                        // we can pass them again to the shell running the exec command.
                        sh(script: 'docker-compose exec web /usr/local/bin/python manage.py migrate --noinput', env: [
                            DJANGO_SECRET_KEY: DJANGO_SECRET_KEY_VAR,
                            DB_NAME: env.DB_NAME,
                            DB_USER: env.DB_USER,
                            DB_PASSWORD: env.DB_PASSWORD,
                ...
                withCredentials([string(credentialsId: 'DJANGO_SECRET_KEY_CREDENTIAL', variable: 'DJANGO_SECRET_KEY_VAR')]) {
                    script {
                        sh(script: 'docker-compose exec web /usr/local/bin/python manage.py collectstatic --noinput', env: [
                            DJANGO_SECRET_KEY: DJANGO_SECRET_KEY_VAR,
                            DB_NAME: env.DB_NAME,
                            DB_USER: env.DB_USER,
                            DB_PASSWORD: env.DB_PASSWORD,
